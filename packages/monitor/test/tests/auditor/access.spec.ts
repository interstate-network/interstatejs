...
import { ExtCodeSizeWitness, ExtCodeHashWitness, ExtCodeCopyWitness, SloadWitness, SstoreWitness, TimestampWitness, NumberWitness, CoinbaseWitness, BalanceWitness, SelfBalanceWitness, ChainidWitness } from '@interstatejs/vm';
...
describe('Auditor: Access Record Errors', () => {
  ...
  async function submitBlock(block: Block): Promise<ChainPegTransactionReceipt> {
    ...
    }).send({ from, gas: 5e6, value: '0x2386f26fc10000' }).getReceipt();
  ...
  it('Catches an sstore operation with a bad refund', async () => {
    const header = blockBuilder.getBlock1Header();
    const address = await blockBuilder.deployTestContract();
    const transactions: Transaction[] = await blockBuilder.getTransactions();
    const fnSig = ABI.encodeFunctionSignature('executeSload()')
    ...
    ((witness.state_access_list[1] as SstoreWitness)).refund = toBn(100);
    await respondToChallenge(block.hash(), txIndex, witness.encode());
    expect(await provableErrorEvent()).to.eq('SSTORE');
    const receipt = await errorProofReceipt();
    expect(receipt.events.BlockReverted).to.not.be.null;
  ...
  it('Catches an exit call record with a bad state root', async () => {
    const header = blockBuilder.getBlock1Header();
    const address = await blockBuilder.deployTestContract();
    const transactions: Transaction[] = await blockBuilder.getTransactions();
    const data = ABI.encodeFunctionSignature('executeWithdrawal()')
    ...
    let index = 0;
    for (; index < witness.state_access_list.length; index++) {
      if (witness.state_access_list[index].opcode.eqn(0xf1)) {
        witness.state_access_list[index].stateRootLeave = new BN(randomHexBuffer(32));
      }
    }
    await respondToChallenge(block.hash(), txIndex, witness.encode());
    expect(await provableErrorEvent()).to.eq('EXIT_CALL');
    const receipt = await errorProofReceipt();
    expect(receipt.events.BlockReverted).to.not.be.null;
    expect(receipt.events.BlockReverted.length).to.eq(1);
    const revertData = receipt.events.BlockReverted[0];
