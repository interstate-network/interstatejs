...
async function compile() {
  if (fs.existsSync('./Testo.json')) return JSON.parse(read('./Testo.json'))
  const result = await easySolc({
    sources: {
      'Testo.sol': { content: read('Testo.sol') },
    },
    name: 'Testo'
  })
  fs.writeFileSync(path.join(__dirname, 'Testo.json'), JSON.stringify(result, null, 2));
  return result;
};
...
async function doTest() {
  ...
  // fs.writeFileSync(path.join(__dirname, 'Testo.evm'), cleanCode(result.deployedBytecode));
  const contract = new Contract(abi);
  const testo = await debugVM.deploy(bytecode);
  const testoData = contract.methods.callOtherGuy().encodeABI();
  const comparison = await debugVM.compare({ to: testo, data: testoData }, false, true);
