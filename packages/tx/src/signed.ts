...
export default class SignedTransaction implements Transaction {
  public type: TransactionType = TransactionType.signed;
   ...
   */
  constructor(
    data: Buffer | PrefixedHexString | BufferLike[] | TxData | SignedTransactionJson = {},
  ...
  encode(includeHash?: boolean, includeType?: boolean, includeFrom?: boolean): SignedTransactionJson {
    ...
    if (includeFrom || this.isFake()) baseData.from = bufferToHex(this.getSenderAddress());
    if (this.isFake()) {
      baseData = {
        ...baseData,
