...
import Transaction from '../src/union'
import { FakeTxData, TxsJsonEntry, VitaliksTestsDataEntry } from './types'
import Common from 'ethereumjs-common'
import { TxData, UnionTransaction, TransactionType } from '../src'

interface TestInput {
  raw: string[];
}
const inTxFixtures: TestInput[] = require('./intxs.json');
...
tape('[Transaction]: Basic functions', function(t) {
  ...
  t.test('should decode transactions', function(st) {
    ...
    inTxFixtures.slice(0, 4).forEach(function(tx: any) {
      const pt = new Transaction(tx.raw)
      st.equal(pt.isIncoming, true)
      st.equal('0x' + pt.from.toString('hex'), tx.raw[0])
      st.equal('0x' + pt.to.toString('hex'), tx.raw[1])
      st.equal('0x' + pt.sourceBlock.toString('hex'), tx.raw[2])
      st.equal('0x' + pt.itxIndex.toString('hex'), tx.raw[3])
      st.equal('0x' + pt.gasLimit.toString('hex'), tx.raw[4])
      st.equal('0x' + pt.value.toString('hex'), tx.raw[5])
      st.equal('0x' + pt.data.toString('hex'), tx.raw[6])
      incomingTransactions.push(pt)
      allTransactions.push(pt)
    })
    st.end()
  })

  t.test('should serialize', function(st: any) {
  ...
  t.test('should encode and decode from json', function(st: any) {
    allTransactions.forEach(function(tx) {
      let _encoded = tx.encode()
      let _decoded = Transaction.fromJSON(_encoded);
      st.deepEqual(_decoded.raw, tx.raw);
      st.equal(_decoded.type, tx.type);
      st.equal(_decoded.hash().toString(), tx.hash().toString());
    })
    st.end()
  })

  t.test('should encode and decode transactions to db json', function(st: any) {
    allTransactions.forEach(function(tx) {
      let _encoded = tx.encodeForDb();
      let _decoded = Transaction.fromJSON(_encoded);
      st.deepEqual(_decoded.raw, tx.raw);
      st.equal(_decoded.type.toString(), tx.type.toString());
      st.equal(_decoded.hash().toString(), tx.hash().toString());
    })
    st.end()
  })

  t.test('should encode and decode transactions to rpc json', function(st: any) {
    allTransactions.forEach(function(tx) {
      let _encoded = tx.toJsonRpc();
      let _decoded = Transaction.fromJSON(_encoded);
      st.deepEqual(_decoded.raw, tx.raw);
      st.equal(_decoded.type, tx.type);
      st.equal(_decoded.hash().toString(), tx.hash().toString());
  ...
  t.test('should validate', function(st) {
    ...
    incomingTransactions.forEach(function(tx) {
  ...
  t.test('should not sign tx', function(st) {
    incomingTransactions.forEach(function(tx, i) {
      st.throws(() => tx.sign(Buffer.from(`0x${'00'.repeat(32)}`)), 'Attempted to sign incoming transaction.')
    })
    st.end()
  })

  t.test('should throw bad types', function(st) {
    incomingTransactions.forEach(function(tx, i) {
      let _encoded = tx.encode()
      st.throws(
        () => UnionTransaction.fromJSON(_encoded, TransactionType.signed),
        `Unexpected transaction type: got ${TransactionType.incoming} expected ${TransactionType.signed}`
      )
    })
    st.end()
  })
  ...
  t.test('should round trip decode a tx', function(st) {
    ...
    const s1 = "0x" + tx.serialize().toString('hex')
    const tx2 = new Transaction(s1)
    const s2 = "0x" + tx2.serialize().toString('hex')
    st.equals(s1, s2)
    st.end()
  })
  ...
  t.test('should return data fee', function(st) {
    ...
    st.equals(incomingTransactions[0].getDataFee().toNumber(), 0)
  ...
  t.test('should return base fee', function(st) {
    ...
    st.equals(incomingTransactions[0].getBaseFee().toNumber(), 0)
    st.end()
  })
  ...
  t.test('should return upfront cost', function(st) {
    ...
    st.equals(incomingTransactions[0].getUpfrontCost().toNumber(), 0)
    st.end()
  })
