...
const lib = (trie) => ({
    del: (key) => new Promise((resolve, reject) => {
        trie.put(toBuffer(key), null, (err, result) => err ? reject(err) : resolve(result));
    }),
    get: (key) => new Promise((resolve, reject) => trie.get(toBuffer(key), (err, result) => err ? reject(err) : resolve(result))),
    put: (key, val) => new Promise((resolve, reject) => trie.put(toBuffer(key), toBuffer(val), (err, result) => err ? reject(err) : resolve(result))),
    prove: async (key) => {
        ...
        try {
            ...
            const simpleProof = await new Promise((resolve, reject) => baseTrie.findPath(key, (err, _, __, proof) => err
                ? reject(err)
            ...
            for (; i < simpleProof.length; i++) {
                ...
                if (node.type === 'branch') {
                    ...
                    if (node.raw[nibbles[nibblesPassed]].length === 0)
                        break;
                    nibblesPassed++;
                }
                else {
                    if (matchingNibbleLength(nibbles.slice(nibblesPassed), node.key) !== node.key.length)
                        break;
                ...
                if (divergentSearch.node.type === 'branch') {
                    ...
                    if (count === 1) {
                        ...
                        if (!Array.isArray(raw)) {
                            ...
                            if (extendPath.length & 0x1)
                                extendPath.push('0');
                            divergent = await new Promise((resolve, reject) => baseTrie.findPath(toBuffer(addHexPrefix(extendPath.join(''))), (err, n, m, path) => err ? reject(err) : resolve(path[path.length - 1].serialize())));
                        }
                    }
                }
            }
            ...
            */
            const pathData = await new Promise((resolve, reject) => baseTrie.findPath(toBuffer(key), (err, _, __, path) => err ? reject(err) : resolve(path)));
