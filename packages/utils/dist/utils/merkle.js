...
exports.MerkleTree = exports.getMerkleProof = exports.getMerkleRoot = void 0;
const keccak256_1 = require("./keccak256");
function getParent(a, b) {
    return keccak256_1.keccak256(Buffer.concat([a, b]));
}
function getMerkleRoot(leaves) {
    ...
    for (let i = 0; i < leaves.length; i++)
        nodes[i] = keccak256_1.keccak256(leaves[i]);
    if (leaves.length == 1)
        return nodes[0];
...
function getMerkleProof(leaves, index) {
    ...
    // Generate the leaves
    for (let i = 0; i < leaves.length; i++) {
        let node = keccak256_1.keccak256(leaves[i]);
        putInLevel(0, node);
        nodes[i] = node;
    }
    //
    if (leaves.length == 1)
        return { root: nodes[0], siblings: [] };
    // Add a defaultNode if we've got an odd number of leaves
    if (nextLevelLength % 2 == 1) {
    ...
    for (let i = 0; i < levels.length - 1; i++) {
        ...
        if (isOdd(numNext))
            siblings.push(levels[i][numNext - 1]);
        else
            siblings.push(levels[i][numNext + 1]);
    }
