...
library RLPAccountLib {
  ...
  function hasEmptyState(Account memory account) internal pure returns (bool) {
    return (
      account.stateRoot == 0xa9a4da177ac3f81cfe85a6767678aabb095aa306e72ab73f5cf0559c56d0a530
    );
  }
  ...
  function defaultAccount() internal pure returns (Account memory account) {
    return Account({
      nonce: 0,
      balance: 0,
  ...
  function decodeAccount(bytes memory encoded) internal pure returns (Account memory account) {
    if (encoded.length == 0) return defaultAccount();
    else if (encoded.length == 32) {
      bytes32 value;
      assembly { value := mload(add(encoded, 32)) }
      require(value == bytes32(0), "Invalid account encoding.");
