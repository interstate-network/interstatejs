...
export function compile(dir: string, fileName?: string | string[], importPath?: string, suppressErrors?: boolean) {
  ...
  if (output.errors) {
    ...
    for (let err of output.errors) {
      if (!suppressErrors && err.errorCode != '1878') console.log(err);
