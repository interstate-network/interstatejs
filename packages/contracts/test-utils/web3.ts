...
export async function getWeb3() {
  ...
  let web3 = new Web3(ganache.provider({
    mnemonic: 'obey east head canoe scrub reopen insect debris rifle butter divide day curve list sight'
  }));
...
export class Deployer {
  ...
  constructor(public web3: Web3, public from: string, public contracts: any) {}
  ...
  async deploy(
    ...
    return (<TransactionReceipt> tx).contractAddress;
  }

  async deployBytecode(bytecode: string): Promise<string> {
    const tx = await this.web3.eth.sendTransaction({
      from: this.from,
      data: bytecode,
      gas: 5e6
