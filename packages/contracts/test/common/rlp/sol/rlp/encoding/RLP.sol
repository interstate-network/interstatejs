...
library RLP {
  ...
  function encodeList(bytes[] memory input) internal pure returns (RLPItem memory) {
    RLPItem[] memory rlpItems = new RLPItem[](input.length);
    for (uint256 i = 0; i < input.length; i++) {
      rlpItems[i] = toRLPItem(input[i]);
    }
    return encodeList(rlpItems);
  }
  ...
  function enterListSelf(Walker memory walker) internal pure /* returns (Walker memory) */ {
    RLP.Walker memory tmp = enterList(walker);
    walker.pointer = tmp.pointer;
    walker.data = tmp.data;
    walker.index = tmp.index;
    walker.state = tmp.state;
  }
