"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var ethereumjs_common_1 = __importDefault(require("ethereumjs-common"));
var tape = require("tape");
var ethereumjs_util_1 = require("ethereumjs-util");
var block_1 = require("../src/block");
tape('[Block]: block functions', function (t) {
    t.test('should test block initialization', function (st) {
        var block1 = new block_1.Block(undefined, { chain: 'ropsten' });
        var common = new ethereumjs_common_1.default('ropsten');
        var block2 = new block_1.Block(undefined, { common: common });
        block1.setGenesisParams();
        block2.setGenesisParams();
        st.strictEqual(block1.hash().toString('hex'), block2.hash().toString('hex'), 'block hashes match');
        st.throws(function () {
            new block_1.Block(undefined, { chain: 'ropsten', common: common });
        }, /not allowed!$/, 'should throw on initialization with chain and common parameter'); // eslint-disable-line
        st.end();
    });
    var testData = require('./testdata.json');
    function testTransactionValidation(st, block) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        st.equal(block.validateTransactions(), true);
                        return [4 /*yield*/, block.genTxTrie()];
                    case 1:
                        _a.sent();
                        st.equal(block.validateTransactionsTrie(), true);
                        st.end();
                        return [2 /*return*/];
                }
            });
        });
    }
    t.test('should test transaction validation', function (st) {
        return __awaiter(this, void 0, void 0, function () {
            var block;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        block = new block_1.Block(ethereumjs_util_1.rlp.decode(testData.blocks[0].rlp));
                        st.plan(2);
                        return [4 /*yield*/, testTransactionValidation(st, block)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    });
    t.test('should test transaction validation with empty transaction list', function (st) {
        return __awaiter(this, void 0, void 0, function () {
            var block;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        block = new block_1.Block();
                        st.plan(2);
                        return [4 /*yield*/, testTransactionValidation(st, block)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    });
    var testData2 = require('./testdata2.json');
    t.test('should test uncles hash validation', function (st) {
        var block = new block_1.Block(ethereumjs_util_1.rlp.decode(testData2.blocks[2].rlp));
        st.equal(block.validateUnclesHash(), true);
        st.end();
    });
    t.test('should test isGenesis (mainnet default)', function (st) {
        var block = new block_1.Block();
        st.notEqual(block.isGenesis(), true);
        block.header.number = Buffer.from([]);
        st.equal(block.isGenesis(), true);
        st.end();
    });
    t.test('should test isGenesis (ropsten)', function (st) {
        var block = new block_1.Block(undefined, { chain: 'ropsten' });
        st.notEqual(block.isGenesis(), true);
        block.header.number = Buffer.from([]);
        st.equal(block.isGenesis(), true);
        st.end();
    });
    var testDataGenesis = require('./genesishashestest.json').test;
    t.test('should test genesis hashes (mainnet default)', function (st) {
        var genesisBlock = new block_1.Block();
        genesisBlock.setGenesisParams();
        var rlp = genesisBlock.serialize();
        st.strictEqual(rlp.toString('hex'), testDataGenesis.genesis_rlp_hex, 'rlp hex match');
        st.strictEqual(genesisBlock.hash().toString('hex'), testDataGenesis.genesis_hash, 'genesis hash match');
        st.end();
    });
    t.test('should test genesis hashes (ropsten)', function (st) {
        var common = new ethereumjs_common_1.default('ropsten');
        var genesisBlock = new block_1.Block(undefined, { common: common });
        genesisBlock.setGenesisParams();
        st.strictEqual(genesisBlock.hash().toString('hex'), common.genesis().hash.slice(2), 'genesis hash match');
        st.end();
    });
    t.test('should test genesis hashes (rinkeby)', function (st) {
        var common = new ethereumjs_common_1.default('rinkeby');
        var genesisBlock = new block_1.Block(undefined, { common: common });
        genesisBlock.setGenesisParams();
        st.strictEqual(genesisBlock.hash().toString('hex'), common.genesis().hash.slice(2), 'genesis hash match');
        st.end();
    });
    t.test('should test genesis parameters (ropsten)', function (st) {
        var genesisBlock = new block_1.Block(undefined, { chain: 'ropsten' });
        genesisBlock.setGenesisParams();
        var ropstenStateRoot = '217b0bbcfb72e2d57e28f33cb361b9983513177755dc3f33ce3e7022ed62b77b';
        st.strictEqual(genesisBlock.header.stateRoot.toString('hex'), ropstenStateRoot, 'genesis stateRoot match');
        st.end();
    });
    t.test('should test toJSON', function (st) {
        var block = new block_1.Block(ethereumjs_util_1.rlp.decode(testData2.blocks[2].rlp));
        st.equal(typeof block.toJSON(), 'object');
        st.equal(typeof block.toJSON(true), 'object');
        st.end();
    });
});
//# sourceMappingURL=block.js.map